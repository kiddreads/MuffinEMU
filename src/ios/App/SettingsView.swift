...
struct SettingsView: View {
    ...
    // The Form below was one ~700-line expression and Swift's type checker gave up on
    // it outright: "the compiler is unable to type-check this expression in reasonable
    // time". SwiftUI's generic ViewBuilder nesting makes inference cost grow sharply
    // with the number of sibling views, so the cure is fewer siblings per expression,
    // not simpler ones. Same views, same order, just grouped.

    @ViewBuilder private var settingsGroup1: some View {
    Section("Appearance") {
    ...
    @ViewBuilder private var settingsGroup2: some View {
    Section {
    ...
    @ViewBuilder private var settingsGroup3: some View {
    Section {
        Text(deviceReport)
    ...
    @ViewBuilder private var settingsGroup4: some View {
    Section {
