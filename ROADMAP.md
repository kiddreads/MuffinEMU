...
- [ ] Implement the real bits of `IOSWindowSystem` (size, canvas recreate) instead of stubs.
  - **Partly done.** Window size, physical size, DPI scale and the frame-rate readout are real. Canvas recreate (rotation, split view) is not: the `CAMetalLayer` frame is set once and nothing re-syncs it, so a size change today leaves a stale drawable.
